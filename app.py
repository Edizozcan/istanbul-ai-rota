...
# --- 3. DİNAMİK DİL DESTEKLİ BİRLEŞİK PDF YAPMA FONKSİYONU ---
def generate_full_travel_booklet(multi_day_plan, start_time_input, end_time_input, genel_toplam_maliyet, seyahat_tarzi, seyahat_hizi, dil):
    ...
    # --- KAPAK ÇİZİM FONKSİYONU ---
    hedef_sehir = multi_day_plan[0]['sehir'] if multi_day_plan else "Avrupa"
    ...
    def kapak_ciz(canvas, doc):
        ...
        try:
            ...
            canvas.setFillColorRGB(0.1, 0.15, 0.3)
            canvas.rect(0, 0, width, height, fill=1, stroke=0)

        canvas.setFillColorRGB(0, 0, 0)
        canvas.rect(0, 0, width, 250, fill=1, stroke=0)

        canvas.setFillColor(colors.white)
        canvas.setFont("Helvetica-Bold", 42)
        canvas.drawString(40, 180, "GLOBAL TRAVEL")
        canvas.drawString(40, 135, "BOOKLET")
        
        canvas.setFont("Helvetica", 18)
        canvas.setFillColor(colors.orange)
        canvas.drawString(40, 95, f"Exclusive Route to: {hedef_sehir.upper()}")
        
        canvas.setFont("Helvetica", 14)
        canvas.setFillColor(colors.lightgrey)
        canvas.drawString(40, 65, f"Estimated Budget: {genel_toplam_maliyet} EUR")
        
        canvas.setFont("Helvetica-Oblique", 10)
        canvas.setFillColor(colors.gray)
        canvas.drawString(40, 30, "Autonomously Generated by Global Route Planner V2")
        canvas.restoreState()
    ...
    doc.build(story, onFirstPage=kapak_ciz)
    buffer.seek(0)
...
def populer_rotalari_getir():
    if supabase is None: return [], None
    try:
        res = supabase.table("sehir_rotalari_cache").select("sehir_adi").execute()
        if not res.data: return [], None
        
        sehirler = [satir["sehir_adi"].split("_")[0].capitalize() for satir in res.data]
        df = pd.DataFrame(sehirler, columns=["Şehir"])
        sayimlar = df["Şehir"].value_counts()
        
        return sayimlar.head(4).index.tolist(), sayimlar
    except:
        return [], None
...
with st.sidebar:
    ...
    populer_liste, _ = populer_rotalari_getir()
    if populer_liste:
        populer_metin = ", ".join(populer_liste)
        st.info(f"🔥 **{'İlham Alın' if uygulama_dili == 'Türkçe' else 'Get Inspired'}:** {populer_metin}")
    ...
    if admin_sifre == "YTU2026":
        st.success("Admin Modu Aktif" if uygulama_dili == "Türkçe" else "Admin Mode Active")
        st.markdown("### 📊 Veritabanı Durumu")
        
        if supabase:
            try:
                _, tum_sayimlar = populer_rotalari_getir()
                toplam_kayit = tum_sayimlar.sum() if tum_sayimlar is not None else 0
                
                st.metric(
                    label="Önbelleğe Alınan Rota", 
                    value=int(toplam_kayit), 
                    delta="Veri birikiyor" if toplam_kayit > 20 else "Optimum Seviye",
                    delta_color="inverse"
                )
                
                if tum_sayimlar is not None and not tum_sayimlar.empty:
                    st.markdown("#### 🏆 Üretim Sıklığı (Şehirler)")
                    st.bar_chart(tum_sayimlar)

...
from reportlab.lib.utils import ImageReader # Kapak fotoğrafı için eklendi
...
def generate_full_travel_booklet(multi_day_plan, start_time_input, end_time_input, genel_toplam_maliyet, seyahat_tarzi, seyahat_hizi, dil):
    ...
    # --- YENİ EKLENEN KAPAK ÇİZİM FONKSİYONU ---
    ...
    # Kapak sayfasında tabloların çıkmaması için ilk sayfayı boş bir Spacer ile başlatıp 2. sayfaya geçiyoruz
    ...
    # Kapak fonksiyonunu sadece ilk sayfada çalışacak şekilde belgeye dahil ettik
    ...
    if not user_ai_prompt:
        ...
        if not istek_listesi:
            ...
            if not multi_day_plan:
                ...
                # Kapak ve tablolar birleşik olarak tek PDF'e yazılıyor
...
if st.session_state.plan_olusturuldu:
    ...
    with col_dl1:
        st.download_button(
            ...
            data=st.session_state.full_pdf_bytes,  # Buton tekrar gerçek ve tam PDF dosyasına bağlandı
